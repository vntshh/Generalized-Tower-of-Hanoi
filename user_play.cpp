...
//Helper function to precalculate the moves so as to find the optimal solution
...
void redraw(void) {
	...
	if(flll == 0) {
		...
		for(int i=0;i<st2+to_num_sz2;i++) glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_24,pp2[i]);
	}
	glEnable(GL_DEPTH_TEST);
	glEnable(GL_LIGHTING);
	glFlush(); 
